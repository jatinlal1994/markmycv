                ...
                <div class="fields-inner">
                  ...
                  <div id="auth-first-step" :class="first_step_status">
                    <form>
                      ...
                      <div class="auth-input-field" :class="username_field_class">
                        <input type="text" ref="username" v-model="username" @input="refreshUsernameError" @focus="refreshUsernameError" :class="username_class" spellcheck="false" placeholder="Username" />
                      ...
                      <div class="first-step-next">
                        <div class="input-email-submit">
                          ...
                          <button type="submit" v-else class="submit-email" @click="processUsername">Next <i class="fas fa-chevron-right"></i></button>
                        </div>
                  ...
                  <div id="auth-password-step" :class="password_step_status">
                    <form @submit="submitPassword">
                      ...
                      <div class="auth-input-field" :class="password_field_class">
                        <input type="password" ref="loginpassword" v-model="password" @input="refreshPasswordError" @focus="refreshPasswordError" :class="password_class" spellcheck="false" placeholder="Password" />
                      ...
                      <div class="first-step-next">
                        <div class="input-email-submit">
                          ...
                          <button v-else class="submit-email" @click="processPassword">Next <i class="fas fa-chevron-right"></i></button>
                        </div>
                  ...
                  <div id="otp-step" :class="otp_step_status">
                    <div class="otp-step-inner">
                      ...
                      <div class="otp-field">
                        <input type="text" @click="refreshOTP" @focus="refreshOTP" v-model="otp" placeholder="XXXXXX" @keypress="onlyNumber" maxlength="6" />
                        <div id="resend-otp" :class="otp_resend_status">
                          <i @click="resendOtp" class="fas fa-redo"></i>
                        </div>
                      </div>
                  ...
                  <!-- End Enter OTP Step -->
                  <!-- Enter OTP Step -->
                  <div id="otp-step" :class="otp_login_step_status">
                    <div class="otp-step-inner">
                      <div class="otp-title"><p>Verify Your Account</p></div>
                      <div class="inbox-icon"><i class="fas fa-envelope"></i></div>
                      <div class="otp-instructions"><p>We have sent an Email containing OTP to the Email ID you just submitted. Please enter the OTP to continue.</p></div>
                      <div class="otp-field">
                        <input type="text" @click="refreshOTP" @focus="refreshOTP" v-model="otp" placeholder="XXXXXX" @keypress="onlyNumber" maxlength="6" />
                        <div id="resend-v-otp" :class="v_otp_resend_status">
                          <i @click="resendVOtp" class="fas fa-redo"></i>
                        </div>
                      </div>
                      <div class="otp-submit-button">
                        <div class="continue-loading" id="otp-loading" v-if="otp_loading"><Loading /></div>
                        <p v-else @click="submitLoginOtp">Submit OTP</p>
                      </div>
                      <div class="otp-submit-error">
                        <p v-text="otp_error"></p>
                      </div>
                  ...
                  <div id="signing-in-step" :class="signing_in_step_status">
                    ...
                    <p class="complete-message">Succesfully Signed In</p>
    ...
    .modal-container{width: 700px;max-width: 700px;height: 400px;box-sizing: border-box;position: relative;
      .modal-box{position: absolute;top: 0;left: 0;width: 100%;height: 100%;padding: 0;border-radius: 5px;overflow: hidden;box-shadow: 0 8px 17px 2px rgba(0,0,0,0.14), 0 3px 14px 2px rgba(0,0,0,0.12), 0 5px 5px -3px rgba(0,0,0,0.2);
        ...
        .fields-col{width: 43%;left: 57%;padding: 25px 35px;box-sizing: border-box;
          ...
          .logo{margin-bottom: 28px;margin-top: 16px;
            i{color: #34495E;font-size: 20px}
          }
          ...
          .input-email-submit, .register-submit{text-align: right;padding-right: 20%;margin-top: 10px;
            p, button{color: #eee;background-color: #3d3d3d;display: inline;font-size: 11px;font-weight: 600;border-radius: 4px;padding: 5px 10px;cursor: pointer;transition: 150ms;
              i{margin-left: 2px;}
          ...
          #otp-step{
            ...
            .otp-field{margin-top: 12px;position: relative;
              ...
              #resend-otp, #resend-v-otp{position: absolute;left: calc(50% + 49px);top: 0;background-color: #1ABC9C;color: #fff;width: 26px;height: 26px;text-align: center;border-radius: 4px;border: 1.5px solid #333;box-sizing: border-box;transition: 150ms;
                i{font-size: 12px;cursor: pointer;cursor: pointer;position: absolute;top: 6px;left: 6px;}
                &.rotating{background-color: #F1C40F;
                  i{animation: rotating 1s linear infinite;}
                }
              }
            }
      ...
      #signing-in-step{
        ...
        #loading-span{display: block;width: 80%;margin: 10px auto 0 auto;height: 5px;border-radius: 2px;overflow: hidden;background-color: #eee;
  ...
  methods: {
    ...
    processUsername () {
      ...
      if (data.username === '') {
        ...
        axios.post('https://api.markmycv.com/auth/user-exists', { 'username': data.username }).then(function (response) {
          if (response.data.exists === false) {
            ...
            setTimeout(function () {
              ...
              setTimeout(function () {
                trigger.$refs.loginpassword.focus()
    ...
    processPassword () {
      ...
      if (data.password === '') {
        ...
        axios.post('https://api.markmycv.com/auth/login', {
          ...
          if (response.data.status === 'success') {
            if (response.data.is_verified) {
              data.password_step_status = 'hidden'
              data.signing_in_step_status = 'hiding'
              setTimeout(function () {
                data.signing_in_step_status = 'show'
                axios.post('https://api.markmycv.com/api/token/', {
                  'username': data.username,
                  'password': data.password
                }).then(function (response) {
                  if (response.status === 200) {
                    setTimeout(function () {
                      ...
                      emitter.$emit('setUsername', data.username)
              ...
              setTimeout(function () {
                data.password_step_status = 'hidden'
                data.otp_login_step_status = 'hiding'
                setTimeout(function () {
                  data.otp_login_step_status = 'show'
                }, 250)
              }, 250)
            }
          } else {
    ...
    processCreateAccount () {
      ...
      if (data.auth_fullname_valid && data.auth_register_username_valid && data.auth_email_valid && data.auth_register_password_valid && data.auth_confirm_password_valid && data.accept_terms) {
        ...
        axios.post('https://api.markmycv.com/auth/create-account', {
    ...
    forgotPassword () {
      ...
      if (data.forgot_field === '') {
        ...
        axios.post('https://api.markmycv.com/auth/reset-password', {
    ...
    submitOtp () {
      ...
      if (data.otp.length === 6) {
        ...
        axios({
          method: 'post',
          url: 'https://api.markmycv.com/auth/submit-otp',
          data: {
            'username': data.register_username,
            'otp': data.otp
          }
      ...
      if (data.otp.length === 6) {
        ...
        axios({
          ...
          if (response.data.status === 'ok') {
            data.otp_login_step_status = 'hiding'
            setTimeout(function () {
              data.otp_login_step_status = 'hidden'
              data.signing_in_step_status = 'hiding'
              setTimeout(function () {
                data.signing_in_step_status = 'show'
                axios.post('https://api.markmycv.com/api/token/', {
    ...
    submitForgotOtp () {
      ...
      axios.post('https://api.markmycv.com/auth/forgot-otp', {
        'value': data.forgot_field,
        'otp': data.forgot_otp
      }).then(function (response) {
        if (response.data.status === 'success') {
    ...
    resendOtp () {
      let data = this.$data
      data.otp_resend_status = 'rotating'
      axios.post('https://api.markmycv.com/auth/resend-otp', {
        'username': data.register_username,
        'emailid': data.email
      }).then(function (response) {
        if (response.data.status === 'success') {
          data.otp_resend_status = ''
        }
      }).catch(function (error) {
        console.log(error)
      }).finally(function () { })
    ...
    resendVOtp () {
      let data = this.$data
      data.v_otp_resend_status = 'rotating'
      axios.post('https://api.markmycv.com/auth/resend-v-otp', {
        'username': data.username
      }).then(function (response) {
        if (response.data.status === 'success') {
          data.v_otp_resend_status = ''
        }
      }).catch(function (error) {
        console.log(error)
      }).finally(function () { })
